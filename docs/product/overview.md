...
- V1 is web-only. There is no companion, desktop app, enrollment, extension, or device wait. Google sign-in and passwordless email codes are supported by in-house server auth; any verified email may sign in. Verified email is the identity key; unverified emails never join an account.
