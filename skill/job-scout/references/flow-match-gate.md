...
Keep: integer `score` 8–10 and match keep.
...
HF6 (`contract-match.md` hard filter 6) → Gaps `match blocked`, drop. Do not re-run Gate 1–5.
match → MatchResult. Malformed or no `match_score` → Gaps. `decision` `skip` → Gaps `match skip`, drop.
