...
Canonical facts for **job-discovery** (list-only scout) and **job-apply**
...
Writes `~/.config/profile-root` to this directory so discovery and apply resolve
`data/*` correctly.

## Fill before a useful run

1. Fact-law files under `data/` (see job-apply Fact law table in the skill).
...
- job-discovery is list-only; job-apply stops at review and waits for an explicit yes.
