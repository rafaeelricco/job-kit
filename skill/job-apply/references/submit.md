...
7. On a CAPTCHA or bot check, list the available tools and skills and check whether one has an explicit CAPTCHA-solving or operator-assist capability. Use one when available; generic browser control alone does not qualify. If no such capability exists, stop and hand the surface to the operator. Resume only after the challenge clears.
