...
`applied via` line. Print `Unconfirmed submit per scout/jobs/{filename}`.
Only a direct operator `<file>` or `<url>` invocation without `--yolo` may
retry the pending event observed at selection; commands generated by prep,
digest `send`, or digest `review` never qualify. A newer pending event
cancels that retry exception.
...
frontmatter values), and either `status:` `applied`, `interview`, or `offer`,
or a log carrying a top-level `submit unconfirmed` line from `job-apply` with
no later `applied via` line. Print `Possible duplicate of scout/jobs/{other}`. Only a direct operator `<file>` or
`<url>` invocation without `--yolo` proceeds past it; commands generated by
prep, digest `send`, or digest `review` never do.
