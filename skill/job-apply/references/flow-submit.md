...
2. At an account wall, sign in when this identity already has an account; otherwise create one with identity fields from the review. If ownership is unknown, try sign-in first. An `email already exists` refusal means sign in, never create a second account. Password, OTP, magic link, or 2FA fields stop once for operator handoff; never invent or persist a secret.
3. Accept required application terms and privacy checkboxes on the application path.
4. Upload every review-named attachment before field entry. Upload or replace the review CV even when the filename matches; a visible filename does not prove the reviewed bytes. If no replacement control exists and the named file is already present, continue. Failed or impossible attachment upload stops.
5. Fill staged review fields. Correct values parsed from the CV with the review values. Leave demographic and EEO rows for `operator`, or stop if the form requires them.
...
7. On a CAPTCHA or any bot check, stop and hand the surface to the operator. Never solve one, before or after the review, and never route it to a CAPTCHA-solving tool or skill even when the runtime offers one. Resume only after the operator clears it.
