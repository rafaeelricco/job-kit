...
| 5   | No other readable dossier with a different normalized `url` and the same `company` and `title` slug (schema-dossier "Filename" slug rule on the frontmatter values), that is not dead-by-log, and whose `status:` is not `rejected` or `dropped` | `Possible duplicate of scout/jobs/{other}`                       |
