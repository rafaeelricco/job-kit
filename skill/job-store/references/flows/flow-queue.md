...
| 5   | No other readable dossier with a different normalized `url` and the same `company` and `title` slug (schema-dossier "Filename" slug rule on the frontmatter values) | `Possible duplicate of scout/jobs/{other}`                       |
