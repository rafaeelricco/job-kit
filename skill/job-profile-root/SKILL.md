...
6. else STOP. Name each attempt (env, each pointer file + line, each default config path, walk start), then point at `job-profile-init` (**create new**, or **register existing** with Activate = Yes). Load `./references/format-recover.md` on STOP or step 2–4 fail. Never scaffold a profile from here.
