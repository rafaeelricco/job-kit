...
`*.md.tmp` during atomic rename, and exclusive lock directories `scout/jobs/*.lock`
(per `job-scout/references/dossier.md`). `data/`, `cv/`, and every other
Profile-root path stay read-only in every phase.
