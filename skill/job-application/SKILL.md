...
description: "Read this when you need to draft, stage, and after explicit approve transmit one job application from a posting. CAPTCHA always human. Use when the user asks to apply, write a cover letter, fill an application form, or Easy Apply; stop at review; on yes transmit (account + terms + Submit when needed) then record success to the scout store."
---
...
7. On explicit yes: pipeline Phase 4 — TRANSMIT (account + terms + Submit as contract
   allows; CAPTCHA always hand off). On transmit success evidence — or when the
   operator confirms they submitted outside the agent: Phase 5 — RECORD.
...
- Pipeline: `./references/pipeline.md` (phases, fit, selection, letter shape, review, transmit, record)
